...
def get_label_anno(label_path):
    ...
    # annotations['name'] = np.array([x[0] for x in content if x[0] == "Person"])
    # annotations['truncated'] = np.array([float(x[1]) for x in content])
    # annotations['occluded'] = np.array([int(x[2]) for x in content])
    # # annotations['alpha'] = np.array([float(x[3]) for x in content])
    # annotations['bbox'] = np.array(
    #     [[float(info) for info in x[3:8]] for x in content if x[0] == "Person"]).reshape(-1, 4)
    # # dimensions will convert hwl format to standard lhw(camera) format.
    # # annotations['dimensions'] = np.array(
    # #     [[float(info) for info in x[8:11]] for x in content]).reshape(
    # #         -1, 3)[:, [2, 0, 1]]
    # annotations['location'] = np.array(
    #     [[float(info) for info in x[8:11]] for x in content if x[0] == "Person"]).reshape(-1, 3)
    # # annotations['rotation_y'] = np.array(
    # #     [float(x[14]) for x in content]).reshape(-1)
    if len(content) != 0 and len(content[0]) == 9:  # have score
      annotations['name'] = np.array([x[0] for x in content if x[0] == "Person"])
      # annotations['truncated'] = np.array([float(x[1]) for x in content])
      # annotations['occluded'] = np.array([int(x[2]) for x in content])
      # annotations['alpha'] = np.array([float(x[3]) for x in content])
      annotations['bbox'] = np.array(
          [[float(info) for info in x[1:5]] for x in content if x[0] == "Person"]).reshape(-1, 4)
      # dimensions will convert hwl format to standard lhw(camera) format.
      # annotations['dimensions'] = np.array(
      #     [[float(info) for info in x[8:11]] for x in content]).reshape(
      #         -1, 3)[:, [2, 0, 1]]
      annotations['location'] = np.array(
        [[np.float(info) for info in x[5:8]] for x in content if x[0] == "Person"]).reshape(-1, 3)
      annotations["score"] = np.array([float(x[8]) for x in content if x[0]=="Person"])
    else:
      annotations['name'] = np.array([x[0] for x in content if x[0] == "Person"])
      annotations['truncated'] = np.array([float(x[1]) for x in content if x[0] == "Person"])
      annotations['occluded'] = np.array([float(x[2]) for x in content if x[0] == "Person"])
      # annotations['alpha'] = np.array([float(x[3]) for x in content])
      annotations['bbox'] = np.array(
          [[float(info) for info in x[3:7]] for x in content if x[0] == "Person"]).reshape(-1, 4)
      # dimensions will convert hwl format to standard lhw(camera) format.
      # annotations['dimensions'] = np.array(
      #     [[float(info) for info in x[8:11]] for x in content]).reshape(
      #         -1, 3)[:, [2, 0, 1]]
      annotations['location'] = np.array(
          [[np.float(info) for info in x[7:10]] for x in content if x[0] == "Person"]).reshape(-1, 3)
